...
_CONSTRAINT_LINE = re.compile(
    r"^[\s\*#>•\-]*\**(?:Final\s+Answer|Draft\s+\d+\s*\([^)]*\)|Draft\s+\d+|Role|Constraint|Goal|Input|Output|Task|Step|Note|Plan|Reasoning|Format|Draft|Material|Question|Answer|Context|Source|Reference|Analysis|Approach|Strategy|Summary|Synthesis|Final|Outline|Thought)\**[\s\d]*[:.].*$",
    re.MULTILINE,
)
