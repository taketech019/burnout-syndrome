...
def render() -> None:
    ...
    with col1:
        primary_disease = st.selectbox(
            "1차 판별 우선 질환 (Gemini 2차 분류용)",
            ["depression", "anxiety", "addiction"],
            help="KlueBERT 1차가 BYPASS 상태라 사용자가 선택. 재학습 후 자동화 예정.",
        )
    with col2:
    ...
    # F1 1차 — KlueBERT 판별 (현재 BYPASS — classifier.py 참조)
    with st.spinner("1차 판별 (KlueBERT)..."):
        cls = classify(transcript)
    st.markdown("### 1차 판별 결과 (KlueBERT)")
    if "error" in cls:
        st.error(cls["error"])
    else:
        cols = st.columns(3)
